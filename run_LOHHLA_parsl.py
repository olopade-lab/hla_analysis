...
def main():
    parser = argparse.ArgumentParser(description='''Script to run LOHHLA with Parsl''')
    parser.add_argument('-t', '--tumor_bam', type=str, required=True, help='''Path of the input tumor bam file(s)''')
    parser.add_argument('-n', '--normal_bam', type=str, required=True, help='''Path of the input tumor bam file(s)''')
...
# --novoDir /usr/ \
# --HLAexonLoc {params.soft}/data/hla.dat


@bash_app    
def LOHHLA(bam, out, threads, graph, inputs=[]):
    import os
    import helper_functions
    sample_id = bam.split("/")[-2].replace("-", "_")
    HLA_script = ('/bin/bash -c "export PATH=/bedtools2-master/bin/:$PATH && '
                    'Rscript {params.soft}/scripts/LOHHLAscript.R '
